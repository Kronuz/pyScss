...
from scss.types import BooleanValue, ColorValue, Number
...
def test_background_brushed():
    libextra.background_brushed(Number(0.5), Number(0.5), ColorValue.from_name('red'), Number(0.5))
...
def test_image_color():
    libextra.image_color(ColorValue.from_rgb(1, 1, 0))
    assert True
